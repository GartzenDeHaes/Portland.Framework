    ...
    public class StraightSkeletonGenerator
    {
        ...
        private bool OffsetAndDetectEvents()
        {
            ...
            foreach (var plan in activePlans)
            {
                ...
                {
                    Debug.WriteLine("Unable to find offset");
                    return false;
                }
        ...
        private void CleanupLoops()
        {
            foreach (var plan in activePlans)
            {
                if (plan.Count == 1)
                {
                    Debug.WriteLine("Invalid plan: " + plan.First);
                }
        ...
        private static bool BisectorBisector(Plan.Vertex vertexA, Plan.Vertex vertexB, out float offset)
        {
            if (Intersect.RayRay(vertexA.position, vertexA.bisector, vertexB.position, vertexB.bisector, out var intersection))
            {
                ...
                {
                    float toIntersection = (float)Vector2.Distance(vertexA.position, vertexB.position);
                ...
                Debug.WriteLine($"Invalid bisector intersection\ntype: {intersection.type}\npointA: {intersection.pointA} pointB: {intersection.pointB}\nbisectorA: {vertexA.bisector} bisectorB:{vertexB.bisector}");
                offset = 0;
                return false;
            }
            offset = 0;
        ...
        {
            float toIntersection = (float)Vector2.Distance(vertex.position, intersection);
        ...
        {
            return toIntersection*(float)Geometry.GetAngleBisectorSin(vertex.angle);
        }
        ...
        private static bool BisectorSegment(Plan.Vertex vertex, Vector2 segmentA, Vector2 segmentB, out float offset)
        {
            if (Intersect.RaySegment(vertex.position, vertex.bisector, segmentA, segmentB, out var intersection))
            {
                if (intersection.type == IntersectionType.Point)
                {
                    ...
                    float toIntersection = (float)Vector2.Distance(vertex.position, intersection.pointA);
                    float intersectionAngle = (float)Vector2.SignedAngle(vertex.bisector, segmentDirection);
                    float intersectionSin = MathF.Sin(intersectionAngle*MathHelper.Deg2Radf);
                    float bisectorSin = (float)Geometry.GetAngleBisectorSin(vertex.angle);
                    offset = toIntersection/(1/intersectionSin + 1/bisectorSin);
                    return true;
                }

                Debug.WriteLine(String.Format("Invalid bisector intersection\ntype: {0}\npointA: {1} pointB: {2}\nray: {3} segmentA:{4} segmentB:{5}",
        ...
        {
            return (a - b).SqrMagnitude < MathHelper.Epsilonf;
        }
